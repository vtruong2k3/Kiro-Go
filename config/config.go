...
type Account struct {
	...
	AuthMethod   string `json:"authMethod"`             // Authentication method: "idc" (AWS IdC), "social" (GitHub/Google), or "external_idp" (Microsoft Entra org SSO)
	...
	// External IdP (Microsoft Entra "Sign in with your organization") fields.
	// For AuthMethod == "external_idp" the access/refresh tokens are issued by
...
func AddAccount(account Account) error {
	...
	if err := Save(); err != nil {
		// Persist failed: roll back the in-memory append so the stored account
		// set stays consistent with what is on disk (Req 3.6). Otherwise the
		// account would linger in memory but never be persisted.
